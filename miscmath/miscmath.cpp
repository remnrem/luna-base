...
double MiscMath::hanning_window(unsigned int n, unsigned int N)
{ 
  return 0.5 * (1.0 - std::cos( ( 2.0 * M_PI * (n+1) ) / double(N+1))); 
}


void MiscMath::hanning_window( std::vector<double> * d)
{
  std::vector<double> w = hanning_window( d->size() );
  for (int i=0;i<d->size();i++) (*d)[i] = (*d)[i] * w[i];
}


std::vector<double> MiscMath::hanning_window(int N, bool periodic )
{

  // default is 'symmetric' window (i.e. periodic = F )
  int half, i, idx, n;

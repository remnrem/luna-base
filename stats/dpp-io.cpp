...
void dpp_io::save(const std::string &f, const dpp_matrix_t &m, int n_features,
                  bool append) {
...
std::vector<dpp_matrix_t> dpp_io::load(const std::string &f,
                                       int expected_n_features) {
  std::vector<dpp_matrix_t> out;

  std::ifstream IN1 =
      LunaIO::open_ifstream(Helper::expand(f), std::ios::binary | std::ios::in);
  ...
  while (1) {
    ...
    if (expected_n_features >= 0 && nf != expected_n_features)
      Helper::halt("data in " + f +
                   " does not match the expected feature schema");
...
dpp_io::load_files(const std::vector<std::string> &files,
                   int expected_n_features) {
  std::vector<dpp_matrix_t> out;

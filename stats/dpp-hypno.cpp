...
{
  if ( three_state ) return { "W" , "R" , "NR" };
  return { "W" , "R" , "N1" , "N2" , "N3" };
}

dpp_hypno::lookup_t::lookup_t( edf_t & edf , const std::string & prefix , bool three_state )
{
  ...
  for (size_t i=0; i<labels.size(); i++)
	...
	Helper::halt( "hypnodensity channel " + ch +
		     " not present in the attached EDF (run POPS ... posterior-channels= first)" );

      signal_list_t sl = edf.header.signal_list( ch );
      if ( sl.size() != 1 ) Helper::halt( "could not resolve hypnodensity channel " + ch );
...
{
  std::vector<double> vals( labels.size() , std::numeric_limits<double>::quiet_NaN() );
  for (size_t i=0; i<labels.size(); i++)
    {
      const int idx = nearest_sample( traces[i].tp , t_tp );
      if ( idx >= 0 ) vals[i] = traces[i].raw[idx];
    }
  return vals;
}

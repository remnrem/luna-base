...
namespace {
bool binary_outcome(const param_t &p) {
  const std::string x = p.value("outcome", true);
  return x == "BINARY";
}

void force_outcome_config(lgbm_t &lg, const param_t &p) {
  if (lg.params.empty())
    lg.params = "learning_rate=0.05 num_leaves=31 min_data_in_leaf=20";
  ...
  out += binary_outcome(p) ? " objective=binary metric=binary_logloss"
...
std::vector<std::string> dpp_fit::feature_labels(const dpp_specs_t &specs) {
  ...
  for (int i = 0; i < (int)specs.specs.size(); i++) {
    ...
    const std::string root =
        s.label_root() + ".w" + Helper::dbl2str(s.window_sec);
...
dpp_fit_t::dpp_fit_t(param_t &param)
  ...
  if (!param.has("outcome"))
    Helper::halt("--dpp-fit requires outcome=quantitative or outcome=binary");
  const std::string outcome = param.value("outcome", true);
  if (outcome != "QUANTITATIVE" && outcome != "BINARY")
    Helper::halt("outcome= must be quantitative or binary");
  ...
  if (param.has("covar")) {
    ...
    for (int i = 0; i < (int)covariate_labels.size(); i++) {
      ...
      if (std::find(covariate_labels.begin(), covariate_labels.begin() + i,
        ...
        Helper::halt("covar= contains duplicate covariate label " +
                     covariate_labels[i]);
    }
  }
  ...
  if ((vector_mode || two_level) && stage_conditioned)
    Helper::halt(
        "DPP vector mode currently uses context features in the vector row and "
        "cannot use stage-conditioned hypno corpus fitting");
  hypno_three_state =
      param.has("hypno-three-state") ? param.yesno("hypno-three-state") : false;

  n_folds = param.has("folds") ? param.requires_int("folds")
                               : (model_set_mode ? 5 : 0);
  save_fold_models =
      param.has("folds-save") ? param.yesno("folds-save") : false;
  ...
  if (model_set_mode && param.has("validation"))
    Helper::halt("fit-spec=/covar= model sets require grouped folds and do not "
                 "use validation=");
  if (n_folds == 1)
    Helper::halt("folds= must be at least 2");

  // only meaningful during CV folds (each has its own validation set
  // attached); a harmless no-op for the final, no-validation fit
  early_stopping_rounds = param.has("early-stopping-rounds")
                              ? param.requires_int("early-stopping-rounds")
                              : (n_folds > 0 ? 20 : 0);
...
void dpp_fit_t::parse_fit_spec() {
  ...
  if (!IN.good())
    Helper::halt("could not open fit-spec= " + file);
  ...
  auto labels = [](const std::string &raw) {
    ...
    for (std::string &s : x) {
      ...
      while (!s.empty() && std::isspace((unsigned char)s.back()))
        s.pop_back();
      if (s.empty())
        Helper::halt("fit-spec= contains an empty covariate label");
    }
    return x;
  };
  ...
  while (std::getline(IN, line)) {
    ...
    if (!(ss >> directive))
      continue;
    if (!(ss >> id))
      Helper::halt("fit-spec= line " + Helper::int2str(line_no) +
                   ": missing identifier");
    ...
    if (directive == "MODEL") {
      ...
      while (ss >> tok) {
        ...
        if (key == "covar")
          ...
          if (val == "T" || val == "1")
            ...
            Helper::halt("fit-spec= line " + Helper::int2str(line_no) +
                         ": sleep= must be T/F or 1/0");
        } else
          Helper::halt("fit-spec= line " + Helper::int2str(line_no) +
                       ": unknown MODEL option " + key);
      }
      for (const model_spec_t &z : model_specs)
        if (z.id == m.id)
          Helper::halt("fit-spec= duplicate MODEL " + m.id);
      if (!m.sleep && m.covariates.empty())
        Helper::halt("fit-spec= MODEL " + m.id +
                     " has neither covariates nor sleep features");
      model_specs.push_back(m);
      ...
      while (ss >> tok) {
        ...
        if (key == "base")
          ...
          Helper::halt("fit-spec= line " + Helper::int2str(line_no) +
                       ": unknown COMPARE option " + key);
      }
      if (c.base == "" || c.add == "" || c.base == c.add)
        Helper::halt("fit-spec= line " + Helper::int2str(line_no) +
                     ": invalid COMPARE models");
      for (const contrast_spec_t &z : contrast_specs)
        if (z.id == c.id)
          Helper::halt("fit-spec= duplicate COMPARE " + c.id);
      contrast_specs.push_back(c);
    } else
      Helper::halt("fit-spec= line " + Helper::int2str(line_no) +
                   ": expected MODEL or COMPARE");
  }
  IN.close();
  if (model_specs.empty())
    Helper::halt("fit-spec= contains no MODEL lines");
  for (const model_spec_t &m : model_specs)
    for (const std::string &c : m.covariates)
      if (std::find(covariate_labels.begin(), covariate_labels.end(), c) ==
          covariate_labels.end())
  ...
  for (const contrast_spec_t &c : contrast_specs) {
    ...
    if (!base || !add)
      Helper::halt("fit-spec= COMPARE " + c.id +
...
void dpp_fit_t::load_corpus() {
  ...
  if (param.has("files"))
    files = param.strvector("files");
  else
    files.push_back(param.requires("data"));

  individuals = dpp_io::load_files(files, -1);
  if (individuals.empty())
    Helper::halt("no individuals loaded from data=/files=");
  ...
  if (n_features <= 0)
    Helper::halt("no valid feature rows found in the loaded corpus");

  logger << "  loaded " << individuals.size() << " individuals, " << n_features
         << " features\n";
}

void dpp_fit_t::build_feature_labels() {
  if (vector_mode) {
    full_labels = dpp_vector::labels(
        param.has("embedding-dim") ? param.requires_int("embedding-dim") : 128,
        param);
  ...
  if ((int)full_labels.size() != n_features)
    Helper::halt("spec= feature count (" +
                 Helper::int2str((int)full_labels.size()) +
                 ") does not match the loaded training corpus (" +
                 Helper::int2str(n_features) + ")");
}
...
void dpp_fit_t::attach_phenotypes() {
  for (int i = 0; i < (int)individuals.size(); i++) {
    ...
    if (!cmd_t::pull_ivar(individuals[i].id, phe_label, &y))
      Helper::halt("no phenotype " + phe_label + " found for individual " +
                   individuals[i].id);
    if (binary_outcome(param) && (y != 0.0 && y != 1.0))
      Helper::halt("binary outcome " + phe_label + " must be coded 0/1; " +
                   individuals[i].id + " has " + Helper::dbl2str(y));
...
void dpp_fit_t::attach_covariates() {
  ...
  for (int i = 0; i < (int)individuals.size(); i++)
    for (int c = 0; c < (int)covariate_labels.size(); c++) {
      ...
      if (cmd_t::pull_ivar(individuals[i].id, covariate_labels[c], &x)) {
        covariate[individuals[i].id][covariate_labels[c]] = x;
        ++n_numeric;
      } else
  ...
  logger
      << "  loaded " << covariate_labels.size() << " covariate labels ("
      << n_numeric
      << " numeric subject values; missing/non-numeric retained as missing)\n";
}

void dpp_fit_t::fit_model_set() {
  if (stage_conditioned)
    Helper::halt("fit-spec= model sets do not support stage-conditioned DPP");
  if (n_folds < 2)
    Helper::halt("fit-spec= requires folds>=2");

  std::map<std::string, int> model_index;
  for (int i = 0; i < (int)model_specs.size(); i++)
    model_index[model_specs[i].id] = i;
  ...
  auto make_matrix =
      [&](const model_spec_t &m, const Eigen::MatrixXd &base,
        ...
        for (int r = 0; r < base.rows(); r++) {
          ...
          for (int j = 0; j < nv; j++) {
            ...
            std::map<std::string, std::map<std::string, double>>::const_iterator
                ii = covariate.find(id);
            if (ii != covariate.end()) {
              std::map<std::string, double>::const_iterator jj =
                  ii->second.find(m.covariates[j]);
              if (jj != ii->second.end())
                x = jj->second;
            }
  ...
  auto train = [&](const model_spec_t &m, const Eigen::MatrixXd &tr,
                   const Eigen::MatrixXd &va, const std::vector<double> &yt,
                   const std::vector<double> &yv, const std::vector<float> &wt,
                   const std::vector<float> &wv,
                   const std::vector<std::string> &labels) {
    std::unique_ptr<lgbm_t> lg(new lgbm_t());
    if (param.has("config"))
      lg->load_config(param.value("config"));
    force_outcome_config(*lg, param);
    lg->qt_mode = true;
    lg->attach_training_matrix(tr);
    if (labels.size() == (size_t)tr.cols())
      lg->set_feature_names(labels);
    lg->attach_training_qts(yt);
    if (va.rows() > 0) {
      lg->attach_validation_matrix(va);
      lg->attach_validation_qts(yv);
    }
    lg->training_weights = wt;
    lg->apply_weights(lg->training, &lg->training_weights);
    if (va.rows() > 0) {
      lg->validation_weights = wv;
      lg->apply_weights(lg->validation, &lg->validation_weights);
    }
    if (param.has("iterations"))
      lg->n_iterations = param.requires_int("iterations");
  ...
  for (int f = 0; f < n_folds; f++) {
    ...
    for (std::map<std::string, int>::const_iterator ii =
             fold_assignment.begin();
         ii != fold_assignment.end(); ++ii)
      if (ii->second == f)
        validation_ids.insert(ii->first);
    ...
    for (int mi = 0; mi < (int)model_specs.size(); mi++) {
      std::vector<std::string> labels;
      Eigen::MatrixXd tr =
          make_matrix(model_specs[mi], Xtrain, train_row_key, &labels);
      Eigen::MatrixXd va =
          make_matrix(model_specs[mi], Xvalid, valid_row_key, &labels);
      std::unique_ptr<lgbm_t> lg =
          train(model_specs[mi], tr, va, ytrain, yvalid, indiv_weight_train,
                indiv_weight_valid, labels);
      Eigen::MatrixXd p = lg->predict(va);
      for (int r = 0; r < p.rows(); r++) {
        const std::string &id = valid_row_key[r].first;
        results[mi].pred[id] += p(r, 0);
        results[mi].n[id]++;
      }
    }
  }
  ...
  if (!CO.good())
    Helper::halt("could not write " + out_root + ".contrasts");
  CO << "# DPP model-set "
        "contrasts\nNAME\tBASE\tADD\tN\tBASE_RMSE\tADD_RMSE\tDELTA_RMSE\tBASE_"
        "R2\tADD_R2\tDELTA_R2\n";
  auto metrics = [&](int mi, const std::set<std::string> &ids) {
    double sy = 0, syy = 0, sse = 0;
    int n = 0;
    for (const std::string &id : ids) {
      if (!results[mi].n.count(id))
        continue;
      double y = phenotype[id], p = results[mi].pred[id] / results[mi].n[id];
      sy += y;
      syy += y * y;
      sse += (y - p) * (y - p);
      ++n;
    ...
    const double rmse =
        n ? std::sqrt(sse / n) : std::numeric_limits<double>::quiet_NaN();
    const double mean = n ? sy / n : 0, tss = syy - n * mean * mean;
    const double r2 =
        n && tss > 0 ? 1 - sse / tss : std::numeric_limits<double>::quiet_NaN();
    return std::make_pair(rmse, r2);
  };
  for (const contrast_spec_t &c : contrast_specs) {
    std::set<std::string> ids;
    for (auto &x : results[model_index[c.base]].pred)
      if (results[model_index[c.add]].pred.count(x.first))
        ids.insert(x.first);
    auto a = metrics(model_index[c.base], ids),
         b = metrics(model_index[c.add], ids);
    CO << c.id << "\t" << c.base << "\t" << c.add << "\t" << ids.size() << "\t"
       << a.first << "\t" << b.first << "\t" << a.first - b.first << "\t"
       << a.second << "\t" << b.second << "\t" << b.second - a.second << "\n";
    std::ofstream PO(
        Helper::expand(out_root + ".contrast." + c.id + ".subject").c_str());
    if (!PO.good())
      Helper::halt("could not write " + out_root + ".contrast." + c.id +
                   ".subject");
    PO << "ID\tY_TRUE\tP_BASE\tP_ADD\tSE_BASE\tSE_ADD\tSE_BASE_MINUS_ADD\n";
    for (const std::string &id : ids) {
      double y = phenotype[id],
             pb = results[model_index[c.base]].pred[id] /
                  results[model_index[c.base]].n[id],
             pa = results[model_index[c.add]].pred[id] /
                  results[model_index[c.add]].n[id],
             eb = (y - pb) * (y - pb), ea = (y - pa) * (y - pa);
      PO << id << "\t" << y << "\t" << pb << "\t" << pa << "\t" << eb << "\t"
         << ea << "\t" << eb - ea << "\n";
    }
    PO.close();
  ...
  for (int mi = 0; mi < (int)model_specs.size(); mi++) {
    std::ofstream OO(
        Helper::expand(out_root + "." + model_specs[mi].id + ".oof.subject")
            .c_str());
    if (!OO.good())
      Helper::halt("could not write " + out_root + "." + model_specs[mi].id +
                   ".oof.subject");
    OO << "# DPP model-set subject-level out-of-fold predictions\n"
       << "# model_id=" << model_specs[mi].id << "\n"
       << "ID\tY_TRUE\tY_PRED\n";
    for (std::map<std::string, double>::const_iterator ii =
             results[mi].pred.begin();
         ii != results[mi].pred.end(); ++ii)
  ...
  for (int mi = 0; mi < (int)model_specs.size(); mi++) {
    ...
    Eigen::MatrixXd tr =
        make_matrix(model_specs[mi], Xtrain, train_row_key, &labels);
    Eigen::MatrixXd empty(0, tr.cols());
    std::vector<double> emptyy;
    std::vector<float> emptyw;
    std::unique_ptr<lgbm_t> lg =
        train(model_specs[mi], tr, empty, ytrain, emptyy, indiv_weight_train,
              emptyw, labels);
    std::string root = Helper::expand(out_root + "." + model_specs[mi].id);
    lg->save_model(root + ".mod");
    std::ofstream M(root + ".dpp");
    M << "# DPP model manifest\n# model_id=" << model_specs[mi].id
      << "\n# sleep=" << (model_specs[mi].sleep ? "T" : "F")
      << "\n# covariates=";
    for (int j = 0; j < (int)model_specs[mi].covariates.size(); j++) {
      if (j)
        M << ",";
      M << model_specs[mi].covariates[j];
    }
  ...
  logger << "  wrote " << model_specs.size() << " DPP model(s) and "
         << contrast_specs.size() << " contrast(s)\n";
}
...
void dpp_fit_t::load_hypno_corpus() {
  ...
  std::vector<std::string> files;
  if (param.has("hypno-files"))
    files = param.strvector("hypno-files");
  else
    files.push_back(param.requires("hypno"));
  ...
  for (int i = 0; i < (int)hypno_individuals.size(); i++)
    hypno_idx[hypno_individuals[i].id] = i;

  for (int i = 0; i < (int)individuals.size(); i++) {
    std::map<std::string, int>::const_iterator ii =
        hypno_idx.find(individuals[i].id);
    if (ii == hypno_idx.end())
      Helper::halt("no hypnodensity corpus entry for individual " +
                   individuals[i].id);
    const dpp_matrix_t &hmat = hypno_individuals[ii->second];
    if (hmat.X.size() != individuals[i].X.size())
      Helper::halt("hypnodensity/feature row-count mismatch for individual " +
                   individuals[i].id);
    ...
    for (int r = 0; r < (int)individuals[i].X.size(); r++)
      if (std::fabs(hmat.time_sec[r] - individuals[i].time_sec[r]) > 1e-6)
        Helper::halt("hypnodensity/feature time misalignment for individual " +
                     individuals[i].id + " at row " + Helper::int2str(r) +
                     " (" + Helper::dbl2str(hmat.time_sec[r]) + " vs " +
                     Helper::dbl2str(individuals[i].time_sec[r]) +
                     ") -- wrong hypno=/hypno-files=?");
  }

  logger << "  loaded hypnodensity corpus for " << stage_labels.size()
         << " stages (" << Helper::stringize(stage_labels) << ")\n";
}
...
void dpp_fit_t::assign_folds() {
  ...
  if (param.has("fold-file")) {
    ...
    if (!IN1.good())
      Helper::halt("could not open fold-file= " + f);
    while (!IN1.eof()) {
      std::string id;
      int fold;
      IN1 >> id;
      ...
      if (fold < 0 || fold >= n_folds)
        Helper::halt("fold-file= " + f + ": fold index " +
                     Helper::int2str(fold) + " out of range for folds=" +
                     Helper::int2str(n_folds) + " (individual " + id + ")");
    ...
    for (int i = 0; i < (int)individuals.size(); i++)
      if (fold_assignment.find(individuals[i].id) == fold_assignment.end())
        Helper::halt("fold-file= " + f +
                     ": no fold assignment for individual " +
                     individuals[i].id);

    logger << "  read fold assignment for " << fold_assignment.size()
           << " individuals from " << f << "\n";
    return;
  }
  ...
  for (int i = 0; i < (int)individuals.size(); i++)
    ids.push_back(individuals[i].id);
  std::sort(ids.begin(), ids.end());

  for (int i = 0; i < (int)ids.size(); i++)
    fold_assignment[ids[i]] = i % n_folds;

  logger << "  assigned " << ids.size() << " individuals to " << n_folds
         << " folds (sorted round-robin)\n";
}

namespace {
bool row_is_usable(const std::vector<double> &row) {
  for (int i = 0; i < (int)row.size(); i++)
    if (Helper::realnum(row[i]))
      return true;
  return false;
}
...
std::vector<double> rank_transform(const std::vector<double> &x) {
  ...
  while (i < n) {
    ...
    const double avg_rank =
        ((i + 1) + (j + 1)) / 2.0; // 1-based, averaged over the tied block
    for (int k = i; k <= j; k++)
      r[idx[k]] = avg_rank;
    i = j + 1;
  }
...
void dpp_fit_t::flatten_and_split() {
  if (param.has("validation")) {
    ...
    if (!IN1.good())
      Helper::halt("could not open " + param.value("validation"));
    while (!IN1.eof()) {
      std::string id;
      IN1 >> id;
      if (IN1.eof())
  ...
  for (int i = 0; i < (int)individuals.size(); i++) {
    const bool is_valid =
        validation_ids.find(individuals[i].id) != validation_ids.end();
    int cnt = 0;
    for (int r = 0; r < (int)individuals[i].X.size(); r++)
      if (row_is_usable(individuals[i].X[r]))
        ++cnt;
    kept_rows[i] = cnt;
    if (is_valid)
      n_valid_rows += cnt;
  ...
  if (n_train_rows == 0)
    Helper::halt("no usable training rows in the loaded corpus");
  ...
  if (stage_conditioned) {
    ...
    if (n_valid_rows > 0)
      Hvalid = Eigen::MatrixXd::Constant(n_valid_rows, n_stages, NaN_value);
  }
  ...
  for (int i = 0; i < (int)individuals.size(); i++) {
    ...
    const dpp_matrix_t *hmat =
        stage_conditioned ? &hypno_individuals[hypno_idx[individuals[i].id]]
                          : NULL;
    ...
    for (int r = 0; r < (int)individuals[i].X.size(); r++) {
      ...
      if (is_valid) {
        for (int c = 0; c < n_features; c++)
          Xvalid(valid_row, c) = individuals[i].X[r][c];
        yvalid[valid_row] = y;
        indiv_weight_valid[valid_row] = w;
        valid_row_key[valid_row] =
            std::make_pair(individuals[i].id, individuals[i].time_sec[r]);
        if (stage_conditioned)
          for (int s = 0; s < n_stages; s++)
            Hvalid(valid_row, s) = hmat->X[r][s];
        ++valid_row;
      } else {
        for (int c = 0; c < n_features; c++)
          Xtrain(train_row, c) = individuals[i].X[r][c];
        ytrain[train_row] = y;
        indiv_weight_train[train_row] = w;
        train_row_key[train_row] =
            std::make_pair(individuals[i].id, individuals[i].time_sec[r]);
        if (stage_conditioned)
          for (int s = 0; s < n_stages; s++)
            Htrain(train_row, s) = hmat->X[r][s];
        ++train_row;
      }
    }

    if (is_valid) {
      istart_valid.push_back(start);
      wtable_valid[start] = w;
    } else {
      istart_train.push_back(start);
      wtable_train[start] = w;
    }
  }

  logger << "  " << n_train_rows << " training rows"
         << (n_valid_rows > 0
                 ? (", " + Helper::int2str(n_valid_rows) + " validation rows")
                 : std::string(""))
...
void dpp_fit_t::apply_row_weights() {
  ...
  lgbm.add_block_weights(lgbm.training, &lgbm.training_weights, istart_train,
                         wtable_train);
  lgbm.apply_weights(lgbm.training, &lgbm.training_weights);

  if (yvalid.size() > 0) {
    lgbm.add_block_weights(lgbm.validation, &lgbm.validation_weights,
                           istart_valid, wtable_valid);
...
void dpp_fit_t::train_booster() {
  ...
  force_outcome_config(lgbm, param);
  ...
  if ((int)full_labels.size() == Xtrain.cols())
    lgbm.set_feature_names(full_labels);
  ...
  if (forced_n_iterations > 0)
    lgbm.n_iterations = forced_n_iterations;
  else if (param.has("iterations"))
...
void dpp_fit_t::train_stage_boosters() {
  ...
  for (int s = 0; s < n_stages; s++) {
    ...
    if ((int)full_labels.size() == Xtrain.cols())
      lg.set_feature_names(full_labels);
    ...
    for (int r = 0; r < Xtrain.rows(); r++) {
      ...
      lg.training_weights[r] =
          Helper::realnum(pp) ? (float)(indiv_weight_train[r] * pp) : 0.0f;
    }
    lg.apply_weights(lg.training, &lg.training_weights);
    ...
    if (yvalid.size() > 0) {
      ...
      for (int r = 0; r < Xvalid.rows(); r++) {
        ...
        lg.validation_weights[r] =
            Helper::realnum(pp) ? (float)(indiv_weight_valid[r] * pp) : 0.0f;
      }
      lg.apply_weights(lg.validation, &lg.validation_weights);
    }
    ...
    if (!forced_stage_n_iterations.empty())
      lg.n_iterations = forced_stage_n_iterations[s];
    else if (param.has("iterations"))
      lg.n_iterations = param.requires_int("iterations");
    ...
    logger << "  stage " << stage_labels[s]
           << ": trained, calibration a=" << calib_a[s] << " b=" << calib_b[s]
           << "\n";
  }
}
...
void dpp_fit_t::cross_validate() {
  ...
  std::vector<std::vector<double>> fold_calib_a(n_stages_cv),
      fold_calib_b(n_stages_cv);
  ...
  for (int f = 0; f < n_folds; f++) {
    ...
    if (stage_conditioned) {
      train_stage_boosters(); // also populates calib_a/calib_b, calibrated
                              // against this fold
      ...
      for (int s = 0; s < n_stages; s++) {
        ...
        const int bi = stage_lgbm[s].best_iteration > 0
                           ? stage_lgbm[s].best_iteration
                           : stage_lgbm[s].n_iterations;
      ...
      for (int s = 0; s < n_stages; s++)
        Ys[s] = stage_lgbm[s].predict(Xvalid);

      for (int r = 0; r < Xvalid.rows(); r++) {
        double sum_pp = 0, z = 0;
        for (int s = 0; s < n_stages; s++) {
          const double pp = Hvalid(r, s);
          if (!Helper::realnum(pp))
            continue;
          sum_pp += pp;
      ...
      train_booster();
      const int bi =
          lgbm.best_iteration > 0 ? lgbm.best_iteration : lgbm.n_iterations;
    ...
    if (save_fold_models) {
      if (stage_conditioned)
        for (int s = 0; s < (int)stage_labels.size(); s++)
          stage_lgbm[s].save_model(Helper::expand(out_root + ".fold" +
                                                  Helper::int2str(f) + "." +
                                                  stage_labels[s] + ".mod"));
      else
        lgbm.save_model(
            Helper::expand(out_root + ".fold" + Helper::int2str(f) + ".mod"));
    }
  ...
  if (!fold_best_iters.empty()) {
    ...
    logger << "  CV-derived n_iterations=" << forced_n_iterations
           << " (median across " << fold_best_iters.size() << " folds)\n";
  }
  ...
  if (n_stages_cv > 0 && !fold_stage_best_iters[0].empty()) {
    ...
    for (int s = 0; s < n_stages_cv; s++) {
      forced_stage_n_iterations[s] =
          (int)std::lround(MiscMath::median(fold_stage_best_iters[s]));

      double sum_a = 0, sum_b = 0;
      for (int i = 0; i < (int)fold_calib_a[s].size(); i++) {
        sum_a += fold_calib_a[s][i];
        sum_b += fold_calib_b[s][i];
      }
      cv_calib_a[s] = sum_a / (double)fold_calib_a[s].size();
      cv_calib_b[s] = sum_b / (double)fold_calib_b[s].size();

      logger << "  stage " << stage_labels[s]
             << ": CV-derived n_iterations=" << forced_stage_n_iterations[s]
  ...
  logger << "  cross-validation (row level): " << oof_rows.size()
...
void dpp_fit_t::write_oof() {
  ...
  if (!O1.good())
    Helper::halt("could not write " + oof_file);
  ...
  for (int i = 0; i < (int)oof_rows.size(); i++)
    O1 << oof_rows[i].id << "\t" << oof_rows[i].time_sec << "\t"
       << oof_rows[i].y_true << "\t" << oof_rows[i].y_pred << "\t"
       << oof_rows[i].fold << "\n";
  O1.close();

  logger << "  wrote " << oof_rows.size() << " out-of-fold predictions to "
         << oof_file << "\n";
}
...
void dpp_fit_t::aggregate_oof_by_subject() {
  ...
  std::map<std::string, double>
      y_true; // constant per individual; last write wins (all identical)
  ...
  for (std::map<std::string, double>::const_iterator ii = sum_pred.begin();
       ii != sum_pred.end(); ++ii) {
    oof_subject_t s;
    s.id = ii->first;
...
void dpp_fit_t::write_oof_subject() {
  ...
  if (!O1.good())
    Helper::halt("could not write " + f);

  O1 << "# DPP cross-validation out-of-fold predictions, aggregated by "
        "subject\n"
     << "# (mean Y_PRED across all of that subject's rows, vs. the true "
        "phenotype)\n"
     << "# phe=" << phe_label << "\n"
  ...
  logger << "  wrote " << oof_subjects.size()
         << " subject-level out-of-fold summaries to " << f << "\n";
}
...
void write_importance_rows(std::ofstream &O1,
  ...
  for (int r = 0; r < (int)idx.size(); r++)
    O1 << (r + 1) << "\t" << labels[idx[r]] << "\t" << gain[idx[r]] << "\t"
       << (int)split[idx[r]] << "\n";
}
} // namespace
...
void dpp_fit_t::write_importance_table(lgbm_t &lg, const std::string &file) {
  const std::vector<double> gain =
      lg.feature_importance(1); // C_API_FEATURE_IMPORTANCE_GAIN
  const std::vector<double> split =
      lg.feature_importance(0); // C_API_FEATURE_IMPORTANCE_SPLIT

  if ((int)gain.size() != (int)full_labels.size())
    Helper::halt("internal error: feature_importance() length does not match "
                 "full_labels");

  std::ofstream O1(file.c_str());
  if (!O1.good())
    Helper::halt("could not write " + file);

  O1 << "# DPP feature importance : " << file << "\n";
  O1 << "# sorted by GAIN (total loss-reduction attributable to splits on that "
        "feature), descending\n";
  O1 << "# SPLIT = number of times the feature was chosen as a split variable "
        "across all trees\n";
...
void dpp_fit_t::write_aggregate_importance_table(const std::string &file) {
  ...
  O1 << "# DPP feature importance (aggregate across stage boosters "
     << Helper::stringize(stage_labels) << ") : " << file << "\n";
  O1 << "# GAIN/SPLIT summed across all stage boosters -- see "
        "<root>.<stage>.importance for per-stage values\n";
  O1 << "# sorted by summed GAIN, descending\n";
  write_importance_rows(O1, full_labels, agg_gain, agg_split);
  O1.close();

  logger << "  wrote aggregate (cross-stage) feature importance table to "
         << file << "\n";
}
...
void dpp_fit_t::save_bundle() {
  if (!stage_conditioned) {
    ...
    if (!O1.good())
      Helper::halt("could not write " + manifest_file);
    ...
    for (int i = 0; i < (int)full_labels.size(); i++)
      O1 << full_labels[i] << "\n";
    O1.close();

    logger << "  wrote model to " << model_file << " and manifest to "
           << manifest_file << "\n";
    return;
  }
  ...
  for (int s = 0; s < (int)stage_labels.size(); s++) {
    stage_lgbm[s].save_model(
        Helper::expand(out_root + "." + stage_labels[s] + ".mod"));
    write_importance_table(
        stage_lgbm[s],
        Helper::expand(out_root + "." + stage_labels[s] + ".importance"));
  }
  ...
  logger << "  wrote " << stage_labels.size()
         << " stage models and manifest to " << manifest_file << "\n";
}
...
manifest_t read_manifest(const std::string &manifest_file) {
  ...
  while (1) {
    ...
    if (line[0] == '#') {
      if (line.find("# mode=") == 0)
        m.mode = line.substr(7);
      else if (line.find("# vector-time=") == 0)
        m.vector_time = line.substr(14);
      else if (line.find("# covariates=") == 0)
        m.covariates = Helper::parse(line.substr(13), ",");
      else if (line.find("# stages=") == 0)
        m.stages = Helper::parse(line.substr(9), ",");
...
void dpp_fit::apply(edf_t &edf, param_t &param, const dpp_specs_t &specs,
  ...
  if (!Helper::fileExists(manifest_file))
  ...
  if (dpp_vector::enabled(param)) {
    ...
    if (requested != manifest.vector_time)
      Helper::halt("DPP model=: vector-time mismatch: model=" +
                   manifest.vector_time + " requested=" + requested);
  }
  if (manifest.mode == "two-level") {
    if (!param.has("covar") && !manifest.covariates.empty()) {
      std::string covar;
      for (const std::string &label : manifest.covariates) {
        if (!covar.empty())
          covar += ",";
        covar += label;
      }
      param.add("covar", covar);
      logger << "  DPP model=: using manifest covariates=" << covar << "\n";
    }
  ...
  if (manifest.feature_labels.size() != expected.size())
    Helper::halt("DPP model=: feature count mismatch: loaded=" +
                 Helper::int2str((int)manifest.feature_labels.size()) +
                 " expected=" + Helper::int2str((int)expected.size()));
  for (int i = 0; i < (int)expected.size(); i++)
    if (manifest.feature_labels[i] != expected[i])
      Helper::halt("DPP model=: feature-name mismatch at column " +
                   Helper::int2str(i + 1) +
                   " (loaded=" + manifest.feature_labels[i] +
                   ", expected=" + expected[i] + ")");
  ...
  if (nr_out == 0) {
    logger << "  *** no DPP output rows to apply model to\n";
    return;
  }
  ...
  if (!stage_conditioned) {
    ...
    if (hmat == NULL)
      Helper::halt("DPP model=: a stage-conditioned model requires "
                   "hypno-prefix= to be supplied");
    if ((int)hmat->time_sec.size() != nr_out)
      Helper::halt("DPP model=: hypnodensity row count does not match the "
                   "feature output row count");

    const int n_stages = (int)manifest.stages.size();
    if (n_stages == 0)
      Helper::halt(
          "DPP model=: stage-conditioned manifest has no stages= list");
    ...
    for (int s = 0; s < n_stages; s++) {
      ...
      const std::string stage_model_file =
          Helper::expand(root + "." + stg + ".mod");
      if (!Helper::fileExists(stage_model_file))
        Helper::halt("could not find " + stage_model_file);
      stage_lgbm[s].qt_mode = true;
      stage_lgbm[s].load_model(stage_model_file);

      std::map<std::string, double>::const_iterator aa =
          manifest.calib_a.find(stg);
      std::map<std::string, double>::const_iterator bb =
          manifest.calib_b.find(stg);
      if (aa != manifest.calib_a.end())
        a[s] = aa->second;
      if (bb != manifest.calib_b.end())
        b[s] = bb->second;
    }

    std::vector<Eigen::MatrixXd> Ys(n_stages);
    for (int s = 0; s < n_stages; s++)
      Ys[s] = stage_lgbm[s].predict(X);

    for (int r = 0; r < nr_out; r++) {
      double sum_pp = 0, z = 0;
      for (int s = 0; s < n_stages; s++) {
        const double pp = s < (int)hmat->X[r].size()
                              ? hmat->X[r][s]
                              : std::numeric_limits<double>::quiet_NaN();
        if (!Helper::realnum(pp))
          continue;
  ...
  if (dpp_vector::enabled(param)) {
    ...
    if (!param.has("step")) {
      if (nr_out >= 2)
        step_sec = mat.time_sec[1] - mat.time_sec[0];
      else
        step_sec = edf.header.record_duration;
    }
    if (step_sec <= 0)
      Helper::halt("DPP vector model application found invalid time points");
  }
  ...
  if (std::fabs(n_spr_d - n_spr) > 1e-6 || n_spr < 1)
    ...
    Helper::halt(
        "DPP model=: record duration (" +
        Helper::dbl2str(edf.header.record_duration) +
  ...
  if (!param.has("dynamic") || param.value("dynamic").empty())
    Helper::halt("DPP model=: dynamic=<signal-label> is required to attach model predictions");
  const std::string label = param.value("dynamic");
  ...
  logger << "  DPP model=: attached signal " << label << " (" << ne_total
         << " samples, " << nr_out << " real values, sentinel=" << SENTINEL
         << ")\n";
}

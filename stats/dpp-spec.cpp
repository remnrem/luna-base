...
namespace {
  ...
  {
    size_t p = s.find( '-' );
    if ( p == std::string::npos || p == 0 || p == s.size() - 1 ) return false;
    if ( ! Helper::str2dbl( s.substr( 0 , p ) , lwr ) ) return false;
    if ( ! Helper::str2dbl( s.substr( p + 1 ) , upr ) ) return false;
    return true;
  }
...
int dpp_spec_t::cols() const
{
  switch ( ftr )
    ...
    case DPP_SLOPE: return 1;
    case DPP_HJORTH: return 3; // activity, mobility, complexity
    case DPP_SKEW: return 1;
    case DPP_KURTOSIS: return 1;
    case DPP_MSE: return 1;
    case DPP_ENVELOPE: return 3; // mean, sd, cv
...
void dpp_specs_t::read( const std::string & f )
{
  ...
  while ( 1 )
	...
	{
	  if ( tok.size() < 2 ) Helper::halt( "expecting: CH label [prefilter=lo-hi]" );
	  dpp_channel_t c;
	  c.ch = tok[1];
	  for (int i=2; i<(int)tok.size(); i++)
	    {
	      std::vector<std::string> kv = Helper::parse( tok[i] , '=' );
	      if ( kv.size() != 2 ) Helper::halt( "bad CH arg: " + tok[i] );
	      if ( Helper::toupper( kv[0] ) == "PREFILTER" )
		{
	      ...
	      else Helper::halt( "unrecognized CH arg: " + kv[0] );
	    }
	  chs[ c.ch ] = c;
	  continue;
	}
	...
	{
	  if ( tok.size() < 4 ) Helper::halt( "expecting: FILTER name lwr upr [ripple=][tw=]" );
	  dpp_filter_t filt;
	  filt.name = tok[1];
	  if ( ! Helper::str2dbl( tok[2] , &filt.lwr ) ) Helper::halt( "bad FILTER lwr: " + line );
	  if ( ! Helper::str2dbl( tok[3] , &filt.upr ) ) Helper::halt( "bad FILTER upr: " + line );
	  filt.ripple = 0.02;
	  filt.tw = 1;
	  for (int i=4; i<(int)tok.size(); i++)
	    {
	      std::vector<std::string> kv = Helper::parse( tok[i] , '=' );
	      if ( kv.size() != 2 ) Helper::halt( "bad FILTER arg: " + tok[i] );
	      if ( Helper::toupper( kv[0] ) == "RIPPLE" )
		{ if ( ! Helper::str2dbl( kv[1] , &filt.ripple ) ) Helper::halt( "bad ripple=: " + kv[1] ); }
	      else if ( Helper::toupper( kv[0] ) == "TW" )
		{ if ( ! Helper::str2dbl( kv[1] , &filt.tw ) ) Helper::halt( "bad tw=: " + kv[1] ); }
	      else Helper::halt( "unrecognized FILTER arg: " + kv[0] );
	    }
	...
	Helper::halt( "expecting colon after block name\n  block: <feature> <channel(s)> [args]" );
      const std::string block = Helper::toupper( block1.substr( 0 , block1.size() - 1 ) );

      if ( tok.size() < 3 ) Helper::halt( "expecting: block: FEATURE channel(s) [args]" );
	...
	{
	  std::vector<std::string> kv = Helper::parse( tok[i] , '=' );
	  if ( kv.size() == 1 ) chan_tokens.push_back( tok[i] );
	  else if ( kv.size() == 2 ) targs[ kv[0] ] = kv[1];
	  else Helper::halt( "bad token: " + tok[i] );
	}
	  ...
	  if ( two_channel )
	    {
	      ...
	      if ( pair.size() != 2 )
		Helper::halt( ftr_lab + " requires a comma-separated channel pair, e.g. C3,C4: " + line );
	      chlabs = pair;
	    }
	  else chlabs.push_back( chan_tokens[c] );
  ...
  for (int i=0; i<(int)channels.size(); i++)
    {
      ...
      dpp_spec_t s1; s1.block = "BASE"; s1.ftr = DPP_PSD;    s1.ch = { c }; s1.window_sec = default_window_sec;
      dpp_spec_t s2; s2.block = "BASE"; s2.ftr = DPP_SLOPE;  s2.ch = { c }; s2.window_sec = default_window_sec;
      dpp_spec_t s3; s3.block = "BASE"; s3.ftr = DPP_HJORTH; s3.ch = { c }; s3.window_sec = default_window_sec;
					  ...
					  const std::string & prefilter_arg )
{
  ...
  if ( filters_arg != "" )
      ...
      for (int i=0; i<(int)toks.size(); i++)
	{
	  ...
	  if ( kv.size() != 2 ) Helper::halt( "bad filters= entry (expecting name:lo-hi): " + toks[i] );
	  double lwr, upr;
	  if ( ! parse_range( kv[1] , &lwr , &upr ) )
	    Helper::halt( "bad filters= range: " + toks[i] );
	  dpp_filter_t filt;
	  filt.name = kv[0]; filt.lwr = lwr; filt.upr = upr; filt.ripple = 0.02; filt.tw = 1;
	  filters[ filt.name ] = filt;
	}
    }
  ...
  if ( features_arg != "" )
    {
      ...
      const std::string default_band = filters.empty() ? "" : filters.begin()->first;
      ...
      for (int i=0; i<(int)toks.size(); i++)
	{
	  ...
	  if ( ftr == DPP_PLV || ftr == DPP_COH || ftr == DPP_PSI ) continue;
	  ...
	  while ( ii != chs.end() )
		...
		{
		  if ( default_band == "" )
		    Helper::halt( "ENVELOPE requested via features= but no filters= band declared" );
		  s.band = { default_band };
		}
	      specs.push_back( s );
	      ++ii;

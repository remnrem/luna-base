...
// coherence_t, psi_t,
// timeline_t::discontinuity/interval_overlaps_masked_region, CHEP's masked())
// -- see stats/dpp-spec.h and stats/dpp-filter.h for the spec grammar and the
// only new filtering glue. No hypnogram_t/NREM-cycle/ hypnodensity reads
// anywhere here (out of scope for DPP itself; see the implementation plan's
// revised SS E/F).
...
bool qc_bad(const std::vector<double> &x, double flat_th, double clip_th,
            double outlier_th) {
  if (x.size() < 2)
    return true;
  if (flat_th > 0 && MiscMath::flat(x) > flat_th)
    return true;
  if (clip_th > 0 && MiscMath::clipped(x) > clip_th)
    return true;
  if (outlier_th > 0) {
    std::vector<bool> inc(x.size(), true);
    const int nout = MiscMath::outliers(&x, outlier_th, &inc);
    if ((double)nout / (double)x.size() > 0.5)
      return true;
  }
  return false;
}
...
bool window_masked(edf_t &edf, uint64_t tp_start, uint64_t tp_stop,
                   const std::string &ch) {
  interval_t iv(tp_start, tp_stop + 1);
  if (edf.timeline.interval_overlaps_masked_region(iv))
    return true;
  ...
  if (edf.timeline.epoched()) {
    ...
    for (int e = e1; e <= e2; e++)
      if (e >= 0 && edf.timeline.masked(e, ch))
        return true;
  }
  return false;
}
...
struct window_result_t {
  ...
  int idx_end;
  window_result_t()
      : ok(false), idx_start_padded(0), idx_start_report(0), idx_end(0) {}
};
...
window_result_t get_window(edf_t &edf, const dpp_trace_t &trace,
                           const std::string &ch, uint64_t t_tp, double w_sec,
                           double pad_sec, bool qc, double qc_flat,
                           double qc_clip, double qc_th) {
  window_result_t r;
  ...
  if (idx_start_padded < 0)
    return r;

  if (timeline_t::discontinuity(trace.tp, trace.sr, idx_start_padded, idx_end))
    return r;
  ...
  if (window_masked(edf, trace.tp[idx_start_padded], trace.tp[idx_end], ch))
    return r;

  if (qc) {
    std::vector<double> rep(trace.effective.begin() + idx_start_report,
                            trace.effective.begin() + idx_end + 1);
    if (qc_bad(rep, qc_flat, qc_clip, qc_th))
      return r;
  }
...
struct dpp_window_cache_t {
  ...
  static std::string key(const std::string &ch, const std::string &band,
                         double window_sec) {
  ...
  entry_t &resolve(edf_t &edf, const dpp_specs_t &specs, const dpp_trace_t &tr,
    ...
    const double pad_sec =
        has_band ? dpp_filters::pad_seconds(*filt, tr.sr) : 0;

    window_result_t wr = get_window(edf, tr, ch, t_tp, window_sec, pad_sec, qc,
                                    qc_flat_th, qc_clip_th, qc_th);
    if (wr.ok) {
      e.ok = true;
      e.drop = wr.idx_start_report - wr.idx_start_padded;
      if (has_band) {
        std::vector<double> padded(tr.effective.begin() + wr.idx_start_padded,
                                   tr.effective.begin() + wr.idx_end + 1);
        e.buf = dpp_filters::apply_band(padded, tr.sr, *filt);
      } else
        e.buf.assign(tr.effective.begin() + wr.idx_start_report,
                     tr.effective.begin() + wr.idx_end + 1);
    }
  ...
  bool get_window_buf(edf_t &edf, const dpp_specs_t &specs,
                      const dpp_trace_t &tr, const std::string &ch,
                      uint64_t t_tp, double window_sec, const std::string &band,
                      bool qc, double qc_flat_th, double qc_clip_th,
                      double qc_th, std::vector<double> *out) {
    entry_t &e = resolve(edf, specs, tr, ch, t_tp, window_sec, band, qc,
                         qc_flat_th, qc_clip_th, qc_th);
    if (!e.ok)
      return false;
  ...
  bool get_hilbert(edf_t &edf, const dpp_specs_t &specs, const dpp_trace_t &tr,
                   const std::string &ch, uint64_t t_tp, double window_sec,
                   const std::string &band, bool qc, double qc_flat_th,
                   double qc_clip_th, double qc_th, std::vector<double> *mag,
    ...
    if (mag)
      mag->assign(e.hmag.begin() + e.drop, e.hmag.end());
    if (phase)
      phase->assign(e.hphase.begin() + e.drop, e.hphase.end());
    return true;
  }
};
...
void welch_params(double w_sec, double *segment_sec, int *noverlap_segments,
                  int n_samples, int sr) {
  *segment_sec = std::min(4.0, w_sec / 2.0);
  ...
  int n = (int)std::floor((n_samples - noverlap_points) /
...
void dsptools::dpp(edf_t &edf, param_t &param) {
  ...
  if (param.has("windows")) {
    ...
    double d;
    if (tok.size() > 0 && Helper::str2dbl(tok[0], &d) && d > 0)
      specs.default_window_sec = d;
  }

  if (param.has("step"))
    specs.default_step_sec = param.requires_dbl("step");
  ...
  if (specs.specs.size() == 0) {
    logger << "  *** no DPP features specified\n";
    return;
  }
  ...
  // off the label) -- fail loudly instead of merging/overwriting silently
  {
    ...
    for (int i = 0; i < (int)specs.specs.size(); i++) {
      const std::string full_label = specs.specs[i].label_root() + ".w" +
                                     Helper::dbl2str(specs.specs[i].window_sec);
      if (seen.find(full_label) != seen.end())
        Helper::halt("duplicate DPP feature specification (two blocks resolve "
                     "to the same label): " +
                     full_label);
  ...
  for (int i = 0; i < (int)specs.specs.size(); i++)
    n_features_total += specs.specs[i].cols();

  const bool any_plv =
      std::any_of(specs.specs.begin(), specs.specs.end(),
                  [](const dpp_spec_t &s) { return s.ftr == DPP_PLV; });

  const double step_sec =
      specs.default_step_sec > 0 ? specs.default_step_sec : 30;
  ...
  const bool show_features =
      param.has("show-features") ? param.yesno("show-features") : false;
  ...
  const double qc_flat_th =
      param.has("qc-flat") ? param.requires_dbl("qc-flat") : 0.05;
  const double qc_clip_th =
      param.has("qc-clip") ? param.requires_dbl("qc-clip") : 0.05;
  ...
  plv_param.amplitude_weighting =
      param.has("plv-weighted") ? param.yesno("plv-weighted") : true;
  plv_param.gate_low_amp =
      param.has("plv-gate") ? param.yesno("plv-gate") : true;
  if (param.has("plv-gate-abs")) {
    ...
    plv_param.gate_quantile =
        param.has("plv-gate-q") ? param.requires_dbl("plv-gate-q") : 0.30;
  ...
  logger << "  DPP options:\n"
         << "    spec        = "
         << (has_spec_file ? param.value("spec") : "(default)") << "\n"
         << "    step        = " << step_sec << "\n"
         << "    show-features = " << (show_features ? "T" : "F") << "\n"
         << "    qc          = " << (qc ? "T" : "F") << "\n";
  if (any_plv)
    logger << "    plv-weighted= "
           << (plv_param.amplitude_weighting ? "T" : "F") << "\n"
           << "    plv-gate    = " << (plv_param.gate_low_amp ? "T" : "F")
           << "\n";
  logger << "    n feature specs   = " << specs.specs.size() << "\n"
         << "    n feature columns = " << n_features_total
         << " (passed to LGBM at --dpp-fit/model= time)\n";
  ...
  while (cc != specs.chs.end()) {
    ...
    if (sl.size() != 1)
      Helper::halt("could not resolve signal " + ch);
    ...
    if (cc->second.has_prefilter)
      tr.effective = dpp_filters::prefilter_trace(tr.raw, tr.tp, tr.sr,
                                                  cc->second.prefilter_lwr,
                                                  cc->second.prefilter_upr);
    else
      tr.effective = tr.raw;
  ...
  for (int i = 0; i < (int)specs.specs.size(); i++) {
    ...
    if (t1.sr != t2.sr)
      Helper::halt("connectivity features require matching sample rates: " +
                   s.ch[0] + ", " + s.ch[1]);
    if (t1.tp.size() != t2.tp.size() ||
        (!t1.tp.empty() &&
         (t1.tp.front() != t2.tp.front() || t1.tp.back() != t2.tp.back())))
      Helper::halt("connectivity features require channels on the same "
                   "timepoint grid (matching start/end/length): " +
                   s.ch[0] + ", " + s.ch[1]);
  }
  ...
  uint64_t last_tp = 0;
  {
    ...
    while (ii != traces.end()) {
      if (!ii->second.tp.empty() && ii->second.tp.back() > last_tp)
        last_tp = ii->second.tp.back();
      ++ii;
    }
  }
  ...
  if (want_hypno) {
    const std::string hypno_prefix =
        param.has("hypno-prefix") ? param.value("hypno-prefix") : "PP";
    const bool hypno_three_state = param.has("hypno-three-state")
                                       ? param.yesno("hypno-three-state")
                                       : false;
    hypno_lookup.reset(
        new dpp_hypno::lookup_t(edf, hypno_prefix, hypno_three_state));
  }
  ...
  for (double t = step_sec; t <= duration_sec; t += step_sec) {
...

    {
      ...
      const double since_heartbeat =
          std::chrono::duration<double>(now_wall - last_heartbeat_wall).count();
      if (since_heartbeat >= 10.0) {
        const double elapsed =
            std::chrono::duration<double>(now_wall - run_start_wall).count();
        ...
        if (t > 0) {
          ...
          eta_str = Helper::dbl2str(std::max(0.0, remaining), 0) +
                    "s (est. total " + Helper::dbl2str(est_total, 0) + "s)";
        }
        logger << "  ... DPP progress: t=" << t << "/" << duration_sec << "s ("
               << Helper::dbl2str(pct, 1)
               << "%), elapsed=" << Helper::dbl2str(elapsed, 1)
    ...
    for (int si = 0; si < (int)specs.specs.size(); si++) {
      ...
      if (verbose)
        logger << "  [t=" << t << "s] " << spec_label << "\n";
      const auto spec_start_wall = std::chrono::steady_clock::now();

      if (spec.ftr == DPP_PSD || spec.ftr == DPP_SLOPE ||
          spec.ftr == DPP_HJORTH || spec.ftr == DPP_SKEW ||
          spec.ftr == DPP_KURTOSIS || spec.ftr == DPP_MSE ||
        ...
        if (win_cache.get_window_buf(edf, specs, tr, spec.ch[0], t_tp,
                                     spec.window_sec, band, qc, qc_flat_th,
                                     qc_clip_th, qc_th, &win)) {
          if (spec.ftr == DPP_PSD || spec.ftr == DPP_SLOPE) {
            double segment_sec;
            int noverlap_segments;
            welch_params(spec.window_sec, &segment_sec, &noverlap_segments,
                         (int)win.size(), tr.sr);
            PWELCH pwelch(win, tr.sr, segment_sec, noverlap_segments,
                          WINDOW_TUKEY50);
            ...
            if (spec.ftr == DPP_PSD) {
              ...
              const double fit_lwr =
                  spec.has("fit-lwr") ? spec.narg("fit-lwr") : 1;
              const double fit_upr =
                  spec.has("fit-upr") ? spec.narg("fit-upr") : 32;
              std::vector<double> fr = {fit_lwr, fit_upr};
              double beta = 0, betan = 0, intercept = 0, rsq = 0;
              const bool sok =
                  spectral_slope_helper(pwelch.psd, pwelch.freq, fr, 0, false,
            ...
            const double EPS = 1e-300;
            vals[0] = log(std::max(act, EPS));
            vals[1] = mob;
            vals[2] = comp;
          } else if (spec.ftr == DPP_SKEW)
            vals[0] = MiscMath::skewness(win);
          else if (spec.ftr == DPP_KURTOSIS)
            vals[0] = MiscMath::kurtosis(win);
            ...
            const bool catch24 =
                spec.has("catch24") && Helper::yesno(spec.arg.at("catch24"));
            catch22_t c22(catch24);
            c22.calc(win.data(), (int)win.size());
            const int n = catch24 ? 24 : 22;
            for (int k = 0; k < n; k++)
              vals[k] = c22.valid() ? c22.stat(k)
        ...
        if (win_cache.get_hilbert(edf, specs, tr, spec.ch[0], t_tp,
                                  spec.window_sec, spec.band[0], qc, qc_flat_th,
                                  qc_clip_th, qc_th, &mag_report, NULL)) {
          const double m = MiscMath::mean(mag_report);
          const double sd =
              mag_report.size() > 1 ? MiscMath::sdev(mag_report, m) : 0;
          vals[0] = m;
          vals[1] = sd;
          vals[2] =
              (m != 0) ? sd / m : std::numeric_limits<double>::quiet_NaN();
        }
        ...
        const bool ok1 = win_cache.get_hilbert(
            edf, specs, tr1, spec.ch[0], t_tp, spec.window_sec, spec.band[0],
            qc, qc_flat_th, qc_clip_th, qc_th, &mag1, &ph1);
        const bool ok2 = win_cache.get_hilbert(
            edf, specs, tr2, spec.ch[1], t_tp, spec.window_sec, spec.band[1],
        ...
        const dpp_trace_t &tr2 = traces[spec.ch[1]];
        window_result_t wr1 =
            get_window(edf, tr1, spec.ch[0], t_tp, spec.window_sec, 0, qc,
                       qc_flat_th, qc_clip_th, qc_th);
        window_result_t wr2 =
            get_window(edf, tr2, spec.ch[1], t_tp, spec.window_sec, 0, qc,
                       qc_flat_th, qc_clip_th, qc_th);
        if (wr1.ok && wr2.ok) {
          std::vector<double> x1(tr1.effective.begin() + wr1.idx_start_report,
                                 tr1.effective.begin() + wr1.idx_end + 1);
          std::vector<double> x2(tr2.effective.begin() + wr2.idx_start_report,
                                 tr2.effective.begin() + wr2.idx_end + 1);
          ...
          if (x1.size() == x2.size()) {
            ...
            coherence_t coh((int)x1.size(), tr1.sr, segment_sec,
                            segment_sec / 2.0);
            coh.clear();
            coh.prepare(1, x1);
            coh.prepare(2, x2);
            ...
            if (spec.band.size() == 1 && spec.band[0] != "") {
              ...
              for (int k = 0; k < (int)frq.size(); k++) {
                if (frq[k] < filt.lwr || frq[k] > filt.upr)
                  continue;
                if (coh.res.bad[k])
                  continue;
      ...
      if (show_features) {
        ...
        for (int k = 0; k < ncol; k++)
          if (Helper::realnum(vals[k]))
            writer.value(ncol == 1 ? "V" : ("V" + Helper::int2str(k + 1)),
                         vals[k]);
  ...
  // actually went without needing to re-run with verbose=T
  {
    ...
    std::sort(order.begin(), order.end(), [&](int a, int b) {
      return spec_time_sec[a] > spec_time_sec[b];
    });
    logger << "  DPP timing (" << Helper::dbl2str(total_wall, 1) << "s total, "
           << specs.specs.size() << " feature specs):\n";
    for (int i = 0; i < (int)order.size(); i++) {
      const int si = order[i];
      const std::string spec_label =
          specs.specs[si].label_root() + ".w" +
          Helper::dbl2str(specs.specs[si].window_sec);
      const double pct =
          total_wall > 0 ? 100.0 * spec_time_sec[si] / total_wall : 0.0;
      logger << "    " << spec_label << " : "
             << Helper::dbl2str(spec_time_sec[si], 2) << "s ("
             << Helper::dbl2str(pct, 1) << "%)\n";
    }
  }
  ...
  if (write_hypno)
    dpp_io::save(param.value("hypno"), hypno_mat,
                 (int)hypno_lookup->labels.size(), false);

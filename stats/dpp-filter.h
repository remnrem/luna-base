...
// Thin glue over the existing FIR *design* machinery
// (dsptools::design_bandpass_fir(), dsp/fir.h) and its own Kaiser
// settling-length calculator (fir_t::calculateKaiserParams, called directly,
// not reimplemented) -- but NOT over dsptools::apply_fir()'s own filter
// *application*. dsptools::apply_fir()/fir_impl_t::filter() deliberately trades
// causality for zero phase shift: it burns in delay_idx=(taps-1)/2 samples
// before recording any output, so output position j is actually the filter's

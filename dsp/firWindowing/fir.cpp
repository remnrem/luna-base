...
void fir_t::demo()
{
  ...
  std::vector<double> lpf_hamming = createWindow(lpf, NULL, windowLength, HAMMING);
  std::vector<double> lpf_blackman = createWindow(lpf, NULL, windowLength, BLACKMAN);
  
  std::vector<double> hpf = create1TransSinc(windowLength, transFreq, sampFreq, HIGH_PASS);
  std::vector<double> hpf_hamming = createWindow(hpf, NULL, windowLength, HAMMING);
  ...
  std::vector<double> bpf_hamming = createWindow(bpf, NULL, windowLength, HAMMING);
  
  std::vector<double> bsf = create2TransSinc(windowLength, trans1Freq, trans2Freq, sampFreq, BAND_STOP);
  std::vector<double> bsf_hamming = createWindow(bsf, NULL, windowLength, HAMMING);
  ...
  std::vector<double> lpf_kaiser = createKaiserWindow(lpf, NULL, kaiserWindowLength, beta);
...
std::vector<double> fir_t::createWindow(double *in, double *out, int windowLength, enum windowType type)
{
...
std::vector<double> createKaiserWindow(double *in, double *out, int windowLength, double beta)
{

...
void edf_t::adjust( param_t & param )
{

  // signals to be adjusted
  signal_list_t signals = header.signal_list( param.value( "sig" ) );

  // (putative) signals to adjust for (i.e. subtract)  
  signal_list_t adjs = header.signal_list( param.value( "adj" ) );
  ...
  // track SR
  const int sigsr = (int)header.sampling_freq( signals(0) );
  ...
  if ( corr_criteria )
	...
	Helper::halt( "corr-sig and corr-th not of similar lengths" );
      ...
      corrsigs = header.signal_list( param.value( "corr-sig" ) );      
    }
  ...
  for (int a=0;a<na;a++)
    if ( header.n_samples[ adjs(a) ] != header.n_samples[ signals(0) ] )
  ...
  for (int c=0;c<nc;c++)
    if ( header.n_samples[ corrsigs(c) ] != header.n_samples[ signals(0) ] )
      Helper::halt( "different SRs, not to RESAMPLE first" );
  
  rec = timeline.first_record();
  while ( rec != -1 )
    {
      ensure_loaded( rec );
      edf_record_t & record = records.find(rec)->second;      
  ...
  for (int s=0; s<ns; s++)
    {
      ...
      // check SR
      if ( (int)header.sampling_freq( signals(s) ) != sigsr )

...
inline int edf_record_t::tc2dec( char a , char b )
{        
  union 
  {
    int16_t       one[1];    
...
inline void edf_record_t::dec2tc( int x , char * a , char * b )
{
  union 
  {
    int16_t       one[1];    
					     ...
					     std::vector<int> * rec ) 
{
  ...
  if ( stop >= timeline.last_time_point_tp )
    {
      // CHECK :: set to this minus 1 ?? 
      stop = timeline.last_time_point_tp;
      //      std::cerr << "moving stop to " << stop << "\n";
    }
      
  //
  // First, figure out which records are being requested 
  //
  ...
  if ( ! okay ) 
    {
      std::cerr << " ** warnign ... null record set found... ** \n";
      return ret; // ie empty
    }
  ...
  while ( r <= stop_record )
	...
	{
	  ret.push_back( record->pdata[ signal ][ s ] );
...
void edf_t::add_signal( const std::string & label , const int Fs , const std::vector<double> & data )
{
  ...
  if ( data.size() != header.nr * n_samples ) 
  ...
  while ( r != -1 ) 
      ...
      for (int i=0;i<n_samples;i++) 
	{
      ...
      records.find(r)->second.add_pdata(t);
...
void edf_record_t::add_pdata( const std::vector<double> & d )
{
  pdata.push_back( d );
}
...
void edf_record_t::add_annot( const std::string & str )
{
  ...
  std::vector<int> dummy; data.push_back(dummy);
...
void edf_t::reference_and_scale( const int s , const int r , const double rescale )
{
  ...
  // reference and rescale; 
  // note, this only impacts the in-memory representation of the physical signal

  // i.e. the digital signal and meta-information are unchanged (i.e. any changes here will
  // not be preserved if the EDF is saved)
  ...
  //
  // for every record, subtract out the reference and rescale (e.g. mV -> uV)
  // 
  ...
  while ( rec != -1 )
	...
	for (int i=0;i<ns;i++) 
	  record.pdata[ s ][i] = 
	    ( record.pdata[ s ][i] - record.pdata[ r ][i] ) * rescale;
      else
	for (int i=0;i<ns;i++) record.pdata[ s ][i] *= rescale;
  ...
  while ( rec != -1 )
      ...
      for (int i=0;i<np;i++) 
	{
	  double refval = 0;
      	  if ( nr == 1 ) 
	    {
	      refval = record.pdata[refs(0)][i];
	    }
	  else
	    {
	      for (int r=0;r<nr;r++) 		
		refval += record.pdata[refs(r)][i];
	      refval /= (double)nr;
	    }
	  
	  // update
	  for (int s=0;s<ns;s++) 
	    record.pdata[signals(s)][i] -= refval;
...
void edf_t::update_physical_minmax( const int s )
{
  ...
  // header.nr * header.n_samples[s]
  double mn = 0 , mx = 0;
  int r = timeline.first_record();
  while ( r != -1 ) 
    {
      for (int p=0;p<header.n_samples[ s ];p++)
	{
	  double x = records.find(r)->second.pdata[ s ][ p ];
	  if ( x < mn ) mn = x;
	  if ( x > mx ) mx = x;
	}
      r = timeline.next_record(r);
  ...
  header.physical_min[s] = mn;
  header.physical_max[s] = mx;  
}
...
void edf_t::update_signal( const int s , std::vector<double> * d )
{
  ...
  // do not want to update header here, as this always refers to READING from the EDF
  // when WRITING the EDF, these are implicitly updated given the new min/max values
  ...
  while ( r != -1 ) 
    {
      ...
      std::vector<int>    & data  = records.find(r)->second.data[ s ];
      
      // check that we did not change sample rate
      
      if ( pdata.size() != points_per_record ) 
	{
...
//   std::cout << "DET0 " << records.begin()->second.pdata.size() << "\t"
...
bool edf_t::attach_index()
{
  return true;
}


bool edf_t::write_index()
{
  ...
  // copy required components from header
  index.header.record_duration = header.record_duration;
  index.header.nr              = header.nr;
  index.header.nr_all          = header.nr_all;

  // Note: a slight kludge.... we need to pre-populate records
  // with exactly 1 empty signal slot (for time-track); as these
  // are cached they will not (i.e. cannot) be re-read from disk (don't 
  // exist on disk).  add_timetrack() will add a new signal however, 
  // so need to reset 'ns' back to 0 before actually calling
  // 
  
  index.header.ns              = 1; 
  index.header.annotation_channel.push_back( true );
  index.header.n_samples.push_back( globals::edf_timetrack_size );

  edf_record_t null_record(&index);
  
  int r = timeline.first_record();
  while ( r != -1 ) 
    {
      index.records.insert( std::map<int,edf_record_t>::value_type (r , null_record ) ) ; // 1 signal
      r = timeline.next_record(r);
    }

  // reset to 0 records
  index.header.ns              = 0; 
  index.header.annotation_channel.clear();
  index.header.n_samples.clear();

  // make EDF plus (this will add a time-track also)
  index.set_edfplus();

  // write to disk
  index.write( filename + ".idx" );
...
void edf_t::rescale( const int s , const std::string & sc )
{
  ...
  if ( ! ( rescale_from_mV_to_uV || rescale_from_uV_to_mV 
	   || rescale_from_V_to_uV || rescale_from_V_to_mV ) ) return;
  ...
  // rescale
  for (int i=0;i<d->size();i++) rescaled[i] = (*d)[i] * fac;
...
bool edf_t::validate( param_t & param )
{
  ...
  for (int s=0; s<ns; s++)
      ...
      for (int i = 0 ; i < n ; i++ )
	{
      ...
      std::vector<double> e_mean, e_sd, e_rms;

...
//       if ( ii->stop  != (*tp1)[ tp1->size()-1 ] ) { ++ii; continue; }
  ...
  for (int s=0;s<nsegs;s++)
	...
	win_start.push_back( index_stop + 1 + w*window_samples );
      ...
      FFT fftseg( index_length , Fs , FFT_FORWARD , WINDOW_HANNING );
      ...
      for (int f=0;f<N;f++)
	{
      ...
      FFT fftall( tot_length , Fs , FFT_FORWARD , WINDOW_HANNING );
      ...
      for (int w=0;w<win_start.size();w++)
	{
	  ...
	  FFT fftwin( window_samples , Fs , FFT_FORWARD , WINDOW_HANNING );      
